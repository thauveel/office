                    ...
                    <tr>
                        <td
                            ...
                            <dl class="font-normal lg:hidden">
                                ...
                                <dd class="mt-1 truncate text-gray-700">{{$job->department->name}}</dd>
                                <dt class="sr-only sm:hidden">Email</dt>
                            </dl>
                        </td>
                        <td class="hidden px-3 py-4 text-sm text-gray-500 lg:table-cell">{{$job->department->name}} in {{$job->department->worksite->name}}</td>
                        <td class="py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 flex justify-between justify-items-center">
                            <a href="{{route('hrm.jobs.edit', compact('job')) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                            <form name="job-{{$job->id}}"
                                action="{{route('hrm.jobs.destroy',compact('job'))}}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button 
